...
class _HistoryViewState extends State<HistoryView> {
  ...
  ({String label, Color fg, Color bg}) _marketplaceStatus(
          ...
          child: SingleChildScrollView(
            ...
            child: DataTable(
              ...
              rows: _evaluations.map((ev) {
                ...
                final disposition =
                    ev['chosenDisposition'] ?? ev['finalDisposition'] ?? '—';
                final dispositionLabel = getPublicDispositionLabel(
                ...
                final heroPhoto = photoUrls != null && photoUrls.isNotEmpty
                    ? photoUrls[0]
                    : null;
                  ...
                  cells: [
                    ...
                    DataCell(
                      ev['feedbackFlag'] == true
                          ? Tooltip(
                              message:
                                  'Marked as ${ev['feedbackType'] ?? 'mis-graded'}',
                              child: Icon(
                                Icons.flag,
                                size: 16,
                    ...
                    DataCell(
                      Row(
                        ...
                        children: [
                          InkWell(
                            onTap: () {
                              final photos = (ev['photoUrls'] as List?)
                                  ?.map((e) => e.toString())
                                  .toList();
                              final e = EvaluationInput(
                                ...
                                condition: condition,
                                conditionReason: ev['conditionReason']
                                    ?.toString(),
                                estimatedResaleValue: resale,
                                finalDisposition: ev['finalDisposition']
                                    ?.toString(),
                                chosenDisposition: ev['chosenDisposition']
                                    ?.toString(),
                                recommendedRoute: ev['recommendedRoute']
                                    ?.toString(),
                                nearestWarehouseId: ev['nearestWarehouseId']
                                    ?.toString(),
                                photoUrls: photos,
                                bestPhotoIndex: ev['bestPhotoIndex'] as int?,
                                distanceKm: ev['distanceKm'] as num?,
                                resaleCount: (ev['resaleCount'] as num?)
                                    ?.toInt(),
                              );
