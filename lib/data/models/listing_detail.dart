  ...
  factory HealthCard.fromJson(Map<String, dynamic> json) {
    return HealthCard(
      ...
      issues: json['issues'] is List
          ? List<String>.from((json['issues'] as List).map((e) => e.toString()))
          : const [],
      ...
      warrantyMonthsRemaining: (json['warrantyMonthsRemaining'] as num?)
          ?.toInt(),
      owners: (json['owners'] as num?)?.toInt() ?? 1,
      circularImpactKg: (json['circularImpactKg'] as num?)?.toDouble(),
      reverseShippingAvoidedKm: (json['reverseShippingAvoidedKm'] as num?)
          ?.toInt(),
